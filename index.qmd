...
top5_bottom5 <- top5_bottom5 %>%
  mutate(friendliness = (`Affectionate With Family` + `Good With Young Children` + `Good With Other Dogs` + `Openness To Strangers` + `Playfulness Level`)/5) %>%
  mutate(upkeep = (`Shedding Level` + `Coat Grooming Frequency` + `Drooling Level`)/3) %>%
  mutate(trainability = (`Trainability Level` + `Adaptability Level`)/2) %>%
  mutate(energy = (`Energy Level` + `Mental Stimulation Needs` + `Barking Level`)/3)
...
ggplot(top5_bottom5, aes(x = xbegin, y = Breed, alpha = 0.5)) + 
  geom_col() +  
  geom_segment(aes(x = xbegin, y = y, xend = x, yend = y)) + 
  geom_point(size = 4, aes(x = friendliness, y = y, color = 'pink', shape = 'circle')) +
  geom_point(size = 4, aes(x = upkeep, y = y, color = 'darkorange', shape = 'square')) +
  geom_point(size = 4, aes(x = energy, y = y, color = 'steelblue2', shape = 'triangle')) +
  geom_point(size = 4, aes(x = trainability, y = y, color = 'tomato', shape = 'diamond')) +
  xlim(min = 1, max = 5) +
  scale_alpha(guide = 'none') +
  labs(title = 'Trait Scores for 5 Most Popular and 5 Least Popular Breeds', subtitle = 'Based on 2020 Rankings', x = 'Score') +
  scale_color_manual(name = 'Trait Groups', 
                     values =c('pink'='pink','darkorange'='darkorange', 'steelblue2'='steelblue2', 'tomato'='tomato'), 
                     labels = c('Friendliness','Upkeep','Energy','Trainability')) +
  scale_shape_manual(name = "Trait Groups",
